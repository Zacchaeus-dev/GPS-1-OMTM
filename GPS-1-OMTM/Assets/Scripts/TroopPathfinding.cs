...
public class TroopClass : MonoBehaviour
{
    ...
    public void SetTroopTargetPosition(Vector2 mP, RaycastHit2D h)
    {
        ...
        hit = h;
            ...
            // 3a. If click on Ground OR ON KD GROUND or Verticallies
            else if (hit.collider != null && hit.collider.CompareTag("[PF] Ground") 
            || hit.collider != null && hit.collider.CompareTag("[PF] KD Ground") 
            || hit.collider != null && hit.collider.CompareTag("[PF] KD Ground (Bug Fixer)")
...
/*        timer = timer + Time.deltaTime;
    ...
    void ClimbAndMove()
    {
        ...
        {
            Debug.Log("wtr");
            transform.position = new Vector2(transform.position.x, transform.position.y + ((moveSpeed + 0.6f) * Time.deltaTime));
